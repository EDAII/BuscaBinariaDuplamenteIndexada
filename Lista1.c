...
int main(int argc, const char * argv[]) {
    ...
    dataArray = populate_data_vector(vectorSize, securityMargin, emptyPositionTracker);
    index2 = populate_second_index_arr(dataArray);
    index1 = populate_first_index_arr(index2);
    
    create_initial_menu(index1, index2, dataArray, emptyPositionTracker);
...
void create_initial_menu(int* index1, int* index2, int* dataArray, int* emptyPositionTracker){
    ...
    do{
        ...
        case '1':
            ...
            if (check_enough_space(emptyPositionTracker, second_index_position)) {
                insert_element_at_position(position, value, dataArray);
                emptyPositionTracker[second_index_position] -= 1;
            } else {
                dataArray = repopulate_vector(dataArray, emptyPositionTracker, index1, index2);
                insert_element_at_position(position, value, dataArray);
                emptyPositionTracker[second_index_position] -= 1;
            }
...
int* repopulate_vector(int* dataArray, int*  emptyPositionTracker, int* index1, int* index2) {
    ...
    int* gappedArray = calloc (vectorSize * 1.2, sizeof(int));
    ...
    for (int i = 0; i < vectorSize; i++, aux++){
        ...
        if( (i % (currentNumbers/100)) == 0) { 
            emptyPositionTracker[aux2++] = currentNumbers/1000;
            aux += currentNumbers/1000;
        }
    }
   free(auxArray);

    vectorSize = vectorSize * 1.1;
...
int* populate_data_vector(int size, float securityMargin, int* emptyPositionTracker){
    ...
    for (int i = 0; i < fullSize ; i++){
        ...
        if ((i % (initialVectorSize/100)) == 0 && i + 10 <= fullSize)
            i += initialVectorSize/1000;
        if(i < fullSize)
            vector[i] = i * 5;  
...
int find_value_position(int* index1, int* index2, int* dataArray, int value, int* second_index_position){
    ...
    *second_index_position = get_second_index_position(index2, value, first_index_position);
    int startPos = *second_index_position * vectorSize/100;
    int endPos = (*second_index_position + 1)  * vectorSize/100;
    while(dataArray[endPos] == 0){
        endPos--;
    }
...
int* populate_second_index_arr(int* dataArray){
    ...
    int offset, value;
    dataArray[0] = 0;
    for(int i = 1; i < size_index_2; i++){   
        offset = 1;
        value = dataArray[i * vectorSize/100];
        while(value == 0){
            value = dataArray[i * vectorSize/100+ offset++];
        }
        index2[i] = value;
    }
    return index2;
...
int get_position_in_data_vector(int* dataArray, int number, int second_index_position){
    ...
    for(int i = startPos; number > dataArray[i] && i < vectorSize; i++){
